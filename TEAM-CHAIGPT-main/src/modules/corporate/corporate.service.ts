...
export class CorporateService {
    ...
    async getRecommendedApplicants(jobId: string, hirerId: string) {
        ...
        const applicantsWithScore: ApplicantMatchResult[] = applicants.map(applicant => {
            ...
            return {
                ...
                totalWeight,
                matchedWeight,
                derivedSkills: applicant.derivedSkills
            };
        });
    ...
    private calculateMatchScore(applicantSkills: string[], requiredSkills: SkillWeight[]): {
        ...
        // Normalize helper: lowercase, remove spaces, hyphens, underscores
        const normalize = (s: string) => s.toLowerCase().replace(/[\s-_]/g, '');
        const normalizedApplicantSkills = new Set(applicantSkills.map(normalize));

        for (const required of requiredSkills) {
            // Check normalized match
            if (normalizedApplicantSkills.has(normalize(required.name))) {
    ...
    async selectApplicant(jobId: string, applicantId: string, hirerId: string) {
        // Verify job belongs to hirer
        const job = await prisma.corporateJob.findFirst({
            where: { id: jobId, hirerId },
            include: { hirer: { select: { name: true } } }
        });
        ...
        const selection = await prisma.jobSelection.upsert({
            where: {
            ...
            include: {
                ...
                applicant: { select: { user: { select: { name: true } } } }
            }
        });
        ...
        await prisma.notification.create({
            data: {
                ...
                message: `${job.hirer.name} has selected you for the "${job.title}" position.`,
                metadata: {
                    jobId: job.id,
                    jobTitle: job.title,
                    hirerName: job.hirer.name
                }
            }
        });
    ...
    // Unselect an applicant from a job
    async unselectApplicant(jobId: string, applicantId: string, hirerId: string) {
        const job = await prisma.corporateJob.findFirst({
            where: { id: jobId, hirerId }
        });
        ...
        await prisma.jobSelection.delete({
            where: {
                jobId_applicantId: { jobId, applicantId }
            }
        }).catch(() => {
            // Selection didn't exist, ignore
        });
    ...
    async getSelectionsForJob(jobId: string, hirerId: string) {
        const job = await prisma.corporateJob.findFirst({
            where: { id: jobId, hirerId }
        });
        ...
        const selections = await prisma.jobSelection.findMany({
            where: { jobId },
            include: {
                applicant: {
                    include: {
    ...
    // Get jobs where applicant has been selected
    async getJobsSelectedForApplicant(applicantUserId: string) {
        const profile = await prisma.applicantProfile.findUnique({
            where: { userId: applicantUserId }
        });
